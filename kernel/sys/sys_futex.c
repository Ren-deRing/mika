...
int64_t sys_futex(uint32_t *uaddr, int op, uint32_t val, const void *timeout, uint32_t *uaddr2, uint32_t val3) {
    ...
    if (cmd == FUTEX_WAIT) {
        ...
        if (timeout != NULL) {
            ...
            uint64_t timeout_ms = (uint64_t)ts.tv_sec * 1000
            ...
            bool woken = (t->t_futex_addr == 0);
            if (t->t_wait_node.next)
                list_del(&t->t_wait_node);
            t->t_futex_addr = 0;
        ...
        while (curr != &g_futex_waiters && woken < (int)val) {
